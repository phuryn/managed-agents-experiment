...
- **OpenAI: no standalone managed-agent product, BUT it does loop server-side over hosted tools (XAI fact-check correction 2026-06-01).** The Responses API runs a server-side loop over its hosted tools (e.g. code_interpreter iterates until completion in a managed container), per OpenAI's Mar 2026 docs ("repeats this loop until the model returns a completion"). So a code task does loop on OpenAI's side. What's MISSING vs Google/Anthropic: a persistent managed-agent product (named agent + session + environment objects) and running YOUR arbitrary tools in a managed loop. The stateful Assistants API (closest to that) is DEPRECATED, sunset 2026-08-26. AgentKit = visual builder, not a hosted agent-loop runtime. So OpenAI's column = hosted-tool economics (the code_interpreter container fee drives the ~16x), NOT managed-agent economics. EARLIER "OpenAI hands the loop back to you / no managed loop" was OVERSTATED -> corrected.
